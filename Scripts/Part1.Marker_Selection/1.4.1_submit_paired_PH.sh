...
while getopts "i:g:t:s:v:" opt; do
  case $opt in
    i) root_dir="$OPTARG"
       ;;
	g) group="$OPTARG"
...
#root_dir="DeepLB/"
output_dir=${root_dir}"/Result/1.4_gen_rand_runs/paired"
#group="PH"
...
do
	echo $runID
	for cancerType in $tumor #'lihc;paad'
	 do 
		mkdir -p ${output_dir}/${cancerType}/${group}
		python 1.4.1_gen_rand_runs_paired.py ${runID} ${cancerType} ${output_dir}/${cancerType}/${group} ${root_dir} ${samplelist}
	done
done
