...
def readData(fileName):
    ...
    with open(fileName) as f:
        for line in f:
            ...
            index = info.pop(0)
            data[index] = info  # np.array(map(float,info))
    return data
...
runID = sys.argv[1]
np.random.seed(20170619 + int(runID))
...
#Note that this is to be modified according to the actual column content of the actual annotation file (if necessary)
seqFileType = {}
    ...
    for sample in seqFileType[type]:
        ...
        # 3	0.867256637168142	113	98	15	57
            ...
            # f.next() # skip the first header line
                next(f)  # File object in Python 3 doesn't support next() method. Python 3 has a built-in function next()
                for line in f:
                    (binIndex,
                    methyValue,
                    allCytosines,
...
methyFile1 = os.path.join(arrayDir_tumor, tumorType + ".tsv")
methyFile2 = os.path.join(arrayDir_tumor, matchedNormaltumorType + ".tsv")
print("\t", tumorType + ".tsv" + " and " + matchedNormaltumorType + ".tsv")
(allMethyTumor, allMethyMatchedNormalTumor, pairedSampleList) = readPairedData(
    methyFile1, methyFile2
)
print(pairedSampleList)
...
# split normal plasma: 32=24+8
# [trainMethy_normal, testMethy_normal] = splitData_v2(allMethyPlasma, trainNormalSampleNames, sampleIndex2Name)
...
# samplelist = trainMethy_tumor.keys()
# samplelist.sort()
for sampleIndex, sampleIndex_matchedNormal in pairedSampleList:
    trainOut.write(
        "\t".join(
            [tumorType]
    ...
    trainMatchedOut.write(
        "\t".join(
            [matchedNormaltumorType]
            + [
                re.sub("nan", "NA", str(x))
                for x in allMethyMatchedNormalTumor[sampleIndex_matchedNormal]
            ]
        )
        + "\n"
    )
    trainOut_SampleList.write(sampleIndex + "\t" + sampleIndex_matchedNormal + "\n")
...
for sampleIndex in trainMethy_normal:
    ...
    trainOut_SampleList.write(sampleIndex + "\tNA\n")
