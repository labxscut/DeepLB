"""Prepare sample lists and DMR inputs for the tumor-only mMTS pipeline.
...
parser.add_argument('-c', '--choose', help='Divide the numbers',default="mean-diff") #freq-diff mean-diff
parser.add_argument('-p', '--purity', help='for tumor purity') #top30 top40 top50 bottom30 bottom40 bottom50
...
all_healthy_blood = meta_data.loc[(meta_data["Group"] == "Healthy") & (meta_data["Sample type"]== "cfDNA")] #21
for i in range(cohort,rep):
    train_list = list_dir + "train_" + str(i)  + ".list"
    train_samples  = pd.read_csv(train_list,delimiter='\t', header=None)
    if purity == "paired":
        train_tumor = train_samples[~train_samples.iloc[:, 0].str.contains('[A-Za-z]')].iloc[:, [0]]
        train_healthy = train_samples[train_samples.iloc[:, 0].str.contains('[A-Za-z]')].iloc[:, [0]]
    else:
...
marker_dir =  env_module.marker_dir + method+ "/"+ find_marker_method + "/"+ tumor_type + "/" + group +"/" + purity + "/"
...
allbeta_file =  env_module.tumor_beta_dir + tumor_type + "_" + purity +".tsv"
    ...
    for index in  tqdm(tumor_sample["sample"], desc=f"Processing files for iteration {i}", leave=False):
        ...
        with open(beta_file, "w") as beta:
            for x in range(0, len(pcc)):
                b = betavalue[x]
                pos = pcc[x].strip("\n").split()
                if pos[1]   == "chrY" or pos[1] == "chrX":# exclude sex chromosomes
                    continue
                elif b == "nan":
                    continue
                else:
